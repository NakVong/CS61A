...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    if (opponent_score // 10 % 10) - (player_score % 10) == 0:
        return 1
    else:
        return 3 * abs((opponent_score // 10 % 10) - (player_score % 10))
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score = 0
    opponent_score = 0
    is_same = strategy(score, opponent_score)
    while score <= goal:
        while opponent_score <= goal:
            if strategy(score, opponent_score) != is_same:
                return False
            opponent_score += 1
        score += 1
        opponent_score = 0
