...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(current_ant) and not current_ant.can_contain(self):
                self.ant_contained = current_ant
                place.ant = self
            elif current_ant.can_contain(self) and not self.can_contain(current_ant):
                current_ant.ant_contained = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            if current_place.bees and self.lower_bound <= transition <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in list(this_place.bees):
...
class HungryAnt(Ant):
    ...
    def devour(self):
        """Devours a random Bee in its place."""
        bee = random_bee(self.place.bees)
        bee.reduce_health(bee.health)

    def action(self, gamestate):
        """Still chewing? Nothing it can do. Done? Feeding Time!"""
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained == None and type(other) != type(self):
            return True
        else:
            return False
        # END Problem 8a
...
class TankAnt(BodyguardAnt):
    """TankAnt deals damage to every Bee in its place each turn."""
    ...
    def action(self, gamestate):
        ...
        while current_place.exit:
            ...
            if current_place.ant:
                ...
                if ant.is_container == True and ant.ant_contained:
