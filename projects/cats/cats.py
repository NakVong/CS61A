...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    def correctness(greater, lesser):
        correct = 0
    ...
    if typed == "" and source == "":
        ...
        return correctness(source_words, typed_words) / len(typed_words) * 100
    else:
        return correctness(typed_words, source_words) / len(typed_words) * 100
    # END PROBLEM 3
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in timestamps_per_player:
        ...
        for i in range(len(player) - 1):
            current_player += [player[i + 1] - player[i]]
        time_per_player += [current_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    result = [[] for k in player_indices]
    for i in word_indices:
        word_time = [time(match, player, i) for player in player_indices]
        used = 0
        for k in player_indices:
            if word_time[k] == min(word_time) and used == 0: # that's the index of the player
                result[k] += [get_word(match, i)]
                used += 1
    return result
    # END PROBLEM 10
